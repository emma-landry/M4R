...
```{r}
data_pres <- read_csv(here("data","kaggle", "president_county_candidate.csv"))
```
...
```{r}
state_policy <-  read_csv(here("data","covid", "state_policy_updates_20210131_1920.csv"))
```
```{r}
mask_data <- read_csv(here("data", "covid", "mask-use-by-county.csv"))
```
```{r}
...
```{r}
new_pres_data <- read_csv(here("data", "election", "2020_US_County_Level_Presidential_Results.csv"))
