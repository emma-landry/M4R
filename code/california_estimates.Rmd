...
```{r}
groups <- readRDS(here("data", "grouped_counties", "california_groups.rds"))
data <- readRDS(here("data", "covid", "prepped_epi_data.rds"))
```
...
for (i in seq (1:N)){
  ...
  #args$init_run <- list(iter = 1e3, control = list(adapt_delta = 0.99), chains=1)
