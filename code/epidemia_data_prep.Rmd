...
```{r}
deaths_all <- read_csv(here("data", "covid", "covid_deaths_usafacts.csv"))

cases_all <- read_csv(here("data", "covid","covid_confirmed_usafacts.csv" ))
```


```{r}
deaths <- deaths_all[,1:318]
cases <- cases_all[,1:318]
```
...
```{r}
saveRDS(data, file = here("data", "grouped_counties", "california_groups.rds"))
```
