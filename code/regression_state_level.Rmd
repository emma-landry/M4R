...
pdf("repvote.pdf", width = 15, height= 10, pointsize = 15)
ggplot() +
  geom_polygon(data= map_plots,mapping = aes(long, lat, group = group, fill = VOTES_2020*100),color = "#ffffff", size = 0.25) +
  geom_text(data = ccdf_labels, aes(long, lat, label = state_abbv), size = 4) +  
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  labs(fill = "Republican Voting (%)")+
  xlab("Longitude")+
  ylab("Latitude")
dev.off()
```
...
```{r}
pdf("hesitancy.pdf", width = 15, height= 10, pointsize = 15)
ggplot() +
  geom_polygon(data= map_plots,mapping = aes(long, lat, group = group, fill = HESIT*100),color = "#ffffff", size = 0.25) +
  geom_text(data = ccdf_labels, aes(long, lat, label = state_abbv), size = 4) +  
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  labs(fill = "Vaccination Hesitancy (%)")+
  xlab("Longitude")+
  ylab("Latitude")
dev.off()
```
```{r}
pdf("rt_map.pdf", width = 15, height= 10, pointsize = 15)
ggplot() +
  geom_polygon(data= map_plots,mapping = aes(long, lat, group = group, fill = VALS3),color = "#ffffff", size = 0.25) +
  geom_text(data = ccdf_labels, aes(long, lat, label = state_abbv), size = 4) +  
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  labs(fill = "Average Rt Nov. \n15 2020- Jan. 15 2020")+
  xlab("Longitude")+
  ylab("Latitude")
dev.off()
```
...
```{r}
lm2 <- lm (VALS3 ~ VOTES_2020+ CHANGE_2020, data = matrix1)
summary(lm2)
```

```{r}
lm3 <- lm(VALS3 ~ HESIT, data= matrix1)
summary(lm3)

ggplot(matrix1, aes(x = HESIT, y = VALS3)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")
```
```{r}
lm4 <- lm(VALS3 ~ NO_GOV_TRUST+ HESIT, data = matrix1)
summary(lm4)
```
```{r}
lm6 <- lm(VALS1 ~  HESIT +VOTES_2020, data = matrix1)
summary(lm6)
```


```{r}
lm5 <- lm(VALS3 ~  HESIT + NO_LIKE+ NO_VAX_TRUST+ NO_GOV_TRUST+ OTHERS_NEED+ VOTES_2020+ CHANGE_2020, data = matrix1)
summary(lm5)
```

```{r}
bayes_lm <- stan_lm (VALS3 ~  HESIT + NO_LIKE+ NO_VAX_TRUST+ NO_GOV_TRUST+ OTHERS_NEED+ VOTES_2020+ CHANGE_2020, data = matrix1, algorithm = "sampling",prior = R2(0.5))
```
```{r}
#summary(bayes_lm)
print(bayes_lm)
