...
ggsave("texas_gp.pdf", height = 5, scale = 1)
```
...
ggplot(joined_california, mapping = aes(long, lat, group = group, fill = as.factor(grouping))) +
  ...
  guides(fill= FALSE)+
  scale_fill_manual(values = mycolors)
ggsave("california_gp.pdf", height = 5, scale = 1)
```
...
ggplot(joined_nebraska, mapping = aes(long, lat, group = group, fill = as.factor(grouping))) +
  ...
  guides(fill= FALSE)+
  scale_fill_manual(values = mycolors)
ggsave("nebraska_gp.pdf", height = 5, scale = 1)
```
...
ggplot(joined_georgia, mapping = aes(long, lat, group = group, fill = as.factor(grouping))) +
  ...
  guides(fill= FALSE)+
  scale_fill_manual(values = mycolors)
ggsave("georgia_gp.pdf", height = 5, scale = 1)
```
```{r}
groups_ohio <- readRDS(here::here("data", "grouped_counties", "ohio_groups.rds"))
N <- nrow(groups_ohio)
tot <- 0
for (i in (1:N)){
  fips <- unlist(groups_ohio$county_fips[i])
  tot <- tot + length(fips)
}
tot
```
...
```{r}
counties_ohio <- counties%>% filter(state_name =="Ohio")
length(unique(counties_ohio$county_fips))
```

```{r}
ohio_group_labels <-data.frame(matrix(ncol = 2, nrow = 159))
colnames(ohio_group_labels) <- c("county_fips", "grouping")
count <- 1
for (i in (1:N)){
  fips <- unlist(groups_ohio$county_fips[i])
  fips_len <- length(fips)
  if (fips_len ==1){
    ohio_group_labels[count,1] <- as.character(fips)
    ohio_group_labels[count,2]<- i
    count <- count +1
  }
  else{
    for (j in (1:fips_len)){
      ohio_group_labels[count,1] <- as.character(fips[j])
      ohio_group_labels[count,2]<- i
      count <- count +1
    }
  }
...
joined_ohio <- ohio_group_labels %>% left_join(counties_ohio, by = "county_fips") 
ggplot(joined_ohio, mapping = aes(long, lat, group = group, fill = as.factor(grouping))) +
  geom_polygon(color = "#ffffff", size = .25) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme_void()+
  theme(legend.title = element_text(),
  ...
  guides(fill= FALSE)+
  scale_fill_manual(values = mycolors)
ggsave("ohio_gp.pdf", height = 5, scale = 1)
```
